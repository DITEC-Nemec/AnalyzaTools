 ...
 */
export function validateImportsExist(model: UnifiedModel): ValidationError[] {
 ...
 */
export function validateNamespaceReferences(model: UnifiedModel): ValidationError[] {
  const errors: ValidationError[] = [];
  ...
  if (model.algorithm) {
    const algorithmErrors = validateModuleNamespaceUsage(
      model.algorithm,
      algorithmImports,
      "algorithm"
    );
 ...
 */
function validateModuleNamespaceUsage(
 ...
 */
function validateEntityNamespaces(
  entity: Entity,
  allowedImports: Set<string>,
  path: string
): ValidationError[] {
  ...
  if (entity.functionList) {
    for (let i = 0; i < entity.functionList.length; i++) {
      ...
      if (func.behavior) {
        const behErrors = validateBehaviorNamespaces(
          func.behavior,
          allowedImports,
          `${path}.functionList[${i}].behavior`
        );
 ...
 */
function validateRelationshipNamespaces(
  rel: Relationship,
  allowedImports: Set<string>,
  path: string
): ValidationError[] {
 ...
 */
function validateAlgorithmNamespaces(
 ...
 */
function validateBehaviorNamespaces(
  behavior: any,
  allowedImports: Set<string>,
  path: string
): ValidationError[] {
  ...
  if (behavior.actorRefList) {
    for (let i = 0; i < behavior.actorRefList.length; i++) {
      const actor = behavior.actorRefList[i] as ActorRef;
 ...
 */
function validateStepNamespaces(
  step: Step,
  allowedImports: Set<string>,
  path: string
): ValidationError[] {
  ...
  if (step.operationRef && typeof step.operationRef === "object") {
    const opErrors = validateOperationNamespaces(
      step.operationRef,
      allowedImports,
      `${path}.operationRef`
    );
  ...
  // Check condition
  if (step.condition) {
    const condErrors = validateConditionNamespaces(
      step.condition,
      allowedImports,
      `${path}.condition`
    );
    ...
    for (let i = 0; i < step.branchList.length; i++) {
      ...
      if (branch.then) {
        for (let j = 0; j < branch.then.length; j++) {
          const subErrors = validateStepNamespaces(
            branch.then[j],
            allowedImports,
            `${path}.branchList[${i}].then[${j}]`
          );
 ...
 */
function validateOperationNamespaces(
 ...
 */
export function validateUnifiedModel(model: UnifiedModel): ValidationResult {
  const errors: ValidationError[] = [];
 ...
 */
export function getValidationErrorsByType(
